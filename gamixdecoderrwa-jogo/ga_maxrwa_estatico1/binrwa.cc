...
//========MixDecoders========================
 ...
 if (chromo[numPath] <= 0.5){ //SPT (Crescente)
   if(chromo[numPath+1] <= 0.5) // ordenacao por caminho minimo e desempate pelas chaves
    creasing(permut, chromo);
   else 			// ordenacao pelas chaves
    creasingkey(permut, chromo);  
  return static_cast<double>(-rpt(permut)); 
 }
 else{ //LPT (decrescente)
   if(chromo[numPath+1] <= 0.5)  // ordenacao por caminho minimo e desempate pelas chaves
    decreasing(permut, chromo);
   else				 // ordenacao pelas chaves
    decreasingkey(permut, chromo);  
  return static_cast<double>(-rpt(permut)); 
 }
...
// Define uma permutacao com ordem crescente de suas chaves
void BinRWA::creasingkey(int permutation[MAXPATH], double *tieBreak) {
  ...
  for (int i = 0; i < numPath; i++) {
    ...
    if (tieBreak == NULL) {
      key[i] = static_cast<double>((rand->IntRand(0, 1000 - 1)));
    } else {
...
// Define uma permutacao com ordem decrescente de suas chaves
void BinRWA::decreasingkey(int permutation[MAXPATH], double *tieBreak) {
