...
do
 echo -e "Instancia $probl $EON_trf\n"
  ./rwa instances/set-d/eon/$probl instances/set-d/eon/$EON_trf GA 1 0 1 >> saida.txt
done
...
do
  ...
  ./rwa instances/set-d/nsf/$probl instances/set-d/nsf/$EON_trf GA 1 0 1 >> saida.txt
done