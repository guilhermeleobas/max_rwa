...
int main(int argc, char** argv) {
  ...
  if (argc != 7){
    cout << "./rwa <network=*.net> <traffic=*.trf> <algorithm= GA|FF|BF|FFD|BFD|RPT|LPT> "
  ...
  char fileresult[] = "testes/result.out";
  char filesol[] = "testes/bestsol.xls";
      ...
      do {
        if (strcmp(alg, "FF") == 0) {
          ...
          cout << "Error: <algorithm> must be "
            << "{GA or FF or BF or FFD or BFD or BFC or RPT or LPT or SPT}\n";
          abort();
        }
        soma += cost;
